...
A small [Pi Coding Agent](https://pi.dev) extension for more stable DeepSeek
requests. "Pi DeepSeek Cache Optimization", pronounced "Pi Disco".

Tested with **Pi 0.84.4** (`@earendil-works/pi-coding-agent`) and Node 26.8.1.
Requires Node 22.19+; other Pi versions are not verified. No runtime dependencies,
provider replacement, background jobs, or extra model calls.

## Install / remove

Try from this checkout without changing Pi settings:

```sh
pi -e ./index.ts
```

For a persistent project-local installation, run these from the target project,
substituting this checkout's absolute path:

```sh
pi install -l /absolute/path/to/pi-dsco
pi remove -l /absolute/path/to/pi-dsco
```

Restart Pi after installation/removal. This repository has not been published.
The development dependencies below are only needed to run checks.
